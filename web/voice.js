...
// Single audio element shared across speak() calls so we can interrupt cleanly.
let _audioEl = null;
// Monotonic generation counter — bumped by stopSpeaking() so any in-flight
// speak() that's still awaiting fetch/blob can detect that it's been
// cancelled mid-flight and bail before constructing the Audio element. Without
// this, clicking mute during the ~200ms fetch+blob window had no effect:
// stopSpeaking() ran with _audioEl still null, then the fetch resolved and
// played anyway.
let _speakGen = 0;
...
export async function speak(text) {
  ...
  // Cancel any in-flight playback. Bumps _speakGen so awaits below see the
  // mismatch and abort.
  stopSpeaking();
  ...
  try {
    ...
    if (myGen !== _speakGen || !ttsEnabled) return;       // cancelled mid-fetch
    if (!resp.ok) {
      console.warn(`tts /tts returned ${resp.status}`);
      // Surface the failure on the button so the operator knows TTS is down.
      const btn = document.getElementById("tts");
      if (btn) {
        const prev = btn.title;
        btn.title = resp.status === 503
          ? "TTS unavailable — Piper not installed. Run: pip install piper-tts && python -m piper --download-dir ~/.severian/voices --download en_GB-alan-medium"
          : `TTS error (HTTP ${resp.status})`;
        setTimeout(() => { btn.title = prev; }, 6000);
      }
      return;
    }
    const blob = await resp.blob();
    if (myGen !== _speakGen || !ttsEnabled) return;       // cancelled mid-blob
    const url  = URL.createObjectURL(blob);
    const audio = new Audio(url);
    audio.addEventListener("ended", () => URL.revokeObjectURL(url));
    if (myGen !== _speakGen || !ttsEnabled) {             // cancelled before play
...
export function stopSpeaking() {
  _speakGen++;   // invalidates any in-flight speak() awaiting fetch/blob
  if (_audioEl) {
    try {
      _audioEl.pause();
      _audioEl.currentTime = 0;
      // Revoke the blob URL so the browser doesn't try to fetch it later
      // when the audio element is GC'd (which would log a stale-blob warning).
